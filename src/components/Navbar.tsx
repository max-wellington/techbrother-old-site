...
export default function Navbar() {
  ...
  // For non-home pages, we want it to be scrolled by default unless we want transparency there too
  // Given the request, "reflecting" the homepage navbar might mean keeping the same behavior
  const activeScrolled = !isHome || scrolled;
      ...
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2">
          <div className="relative w-56 h-14">
            <Image 
              src={activeScrolled ? LOGO_WHITE_URL : LOGO_BLACK_URL}
          ...
          <div className="relative" ref={dropdownRef} onMouseEnter={() => setServicesOpen(true)} onMouseLeave={() => setServicesOpen(false)}>
            <button 
              className={`transition-colors font-semibold flex items-center gap-1 py-2 ${
                activeScrolled ? "text-white/90 hover:text-white" : "text-[#041324] hover:text-primary"
              }`}
            >
              Services
              ...
              {servicesOpen && (
                <motion.div 
                  ...
                  className={`absolute top-full left-0 w-64 border rounded-xl shadow-2xl py-3 overflow-hidden ${
                    activeScrolled ? "bg-[#041324] border-white/10" : "bg-white border-border"
                  }`}
                >
                  {services.map((service) => (
                    <Link
                      ...
                      className={`block px-5 py-3 transition-colors group ${
                        activeScrolled ? "hover:bg-white/5" : "hover:bg-[#041324]/5"
                      }`}
                    >
                      <div className={`font-semibold transition-colors ${
                        activeScrolled ? "text-white group-hover:text-primary" : "text-[#041324] group-hover:text-primary"
                      }`}>{service.name}</div>
                      <div className={`text-xs ${
                        activeScrolled ? "text-white/50 group-hover:text-white/70" : "text-[#041324]/50 group-hover:text-[#041324]/70"
        ...
        <Button 
          variant={activeScrolled ? "secondary" : "default"} 
          asChild 
